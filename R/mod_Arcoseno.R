...
#' Arcoseno Server Functions
#'
#' @noRd
mod_Arcoseno_server <- function(id,file_data){
  moduleServer(id, function(input, output, session){
    ...
    res1_data <- reactive({
      ...
      if (m$common == TRUE){
        ...
        res1 <- dplyr::select( res1, "event.e", "n.e","event.c","n.c","studlab","TE","seTE","lower","upper","zval","pval","w.common", "Ponderacion")
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "ASD", "EE.ASD", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
      ...
      if (m$common == FALSE){
        ...
        res1 <- dplyr::select( res1, "event.e", "n.e","event.c","n.c","studlab","TE","seTE","lower","upper","zval","pval","w.random", "Ponderacion")
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "ASD", "EE.ASD", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
    })
      ...
      if (m$common == TRUE){
        res8dt <-
          data.frame(
            ...
            Estimacion = m[["TE.common.w"]],
            EE = m[["seTE.common.w"]],
            `LI[95%]` = m[["lower.common.w"]],
            `LS[95%]` = m[["upper.common.w"]],
    ...
    res_3_data <- reactive({
      ...
      if(m$common == TRUE){
        ...
        colnames(res3)=c("ASD", "LI[95%]", "LS[95%]", "Z", "valor-p")
        return(res3)
      }
      if(m$common == FALSE){
    ...
    output$tables <-  renderUI({
      ...
      model_summary <- DT::formatRound(
        model_summary,
        columns = c("E.E", "N.E", "E.C", "N.C", "ASD", "EE.ASD",
      ...
      if (!is.null(m$subgroup)){
        ...
        tagList(
          h3(if(m$common == TRUE)
          {"Tabla 1. Diferencia de Arcoseno con Efectos Fijos"}
          else {"Tabla 1. Diferencia de Arcoseno con Efectos Aleatorios"}
          ),
    ...
    output$download_res1 <- downloadHandler(
      filename = function() {
        paste("tabla_diferencia_Arcoseno_", Sys.Date(), ".csv", sep = "")
      },
...
# mod_Arcoseno_server("Arcoseno_1")
