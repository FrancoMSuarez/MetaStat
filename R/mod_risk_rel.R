  ...
  tagList(
    ...
    bslib::layout_sidebar(
      sidebar = bslib::sidebar(
        ...
        selectInput(ns("E_e"),
                    "Seleccionar Ee (Número de eventos grupo experimental)",
                    choices = c("",NULL),
                    selected = ""),

        selectInput(ns("N_e"),
                    "Seleccionar Ne (Número de observaciones grupo experimental)",
                    choices = c("",NULL),
                    selected = ""),

        selectInput(ns("E_c"),
                    "Seleccionar Ec (Número de eventos grupo control)",
                    choices = c("",NULL),
                    selected = ""),

        selectInput(ns("N_c"),
                    "Seleccionar Nc (Número de observaciones grupo control)",
                    choices = c("",NULL),
                    selected = ""),
...
#' risk_rel Server Functions
#'
#' @noRd
mod_risk_rel_server <- function(id, file_data){
  moduleServer(id, function(input, output, session){
    ...
    observe({
      ...
      updateSelectInput(session, "E_e", choices = c("",var_num))
      updateSelectInput(session, "N_e", choices = c("",var_num))
      updateSelectInput(session, "E_c", choices = c("",var_num))
    ...
    model <- eventReactive(input$run_model, {
      df <- file_data()
      req(df)
      req(input$E_e, input$N_e, input$E_c, input$N_c)
      ...
      if (!is.null(input$author) && input$author != "") {
        if (!is.null(input$año) && input$año != "") {
          ...
          metaanalisis_df$Author = df[[input$author]]
        }
      }

      cols_to_keep <- setdiff(names(df), c(input$E_e, input$N_e, input$E_c, input$N_c, input$Sub, input$author, input$año))
    ...
    res1_data <- reactive({
      ...
      if (m$common == TRUE){
        ...
        res1$Te_t=exp(res1$TE)
        res1$seTE_t=exp(res1$seTE)
        res1$lower_tr=exp(res1$lower)
        res1$upper_tr=exp(res1$upper)
        res1 <- dplyr::select( res1, "event.e", "n.e","event.c","n.c","studlab","Te_t","seTE_t","lower_tr","upper_tr","zval","pval","w.common", "Ponderacion")
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "RR", "EE.RR", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
      ...
      if (m$common == FALSE){
        ...
        res1$lower_tr=exp(res1$lower)
        res1$upper_tr=exp(res1$upper)
        res1 <- dplyr::select( res1, "event.e", "n.e","event.c","n.c","studlab","Te_t","seTE_t","lower_tr","upper_tr","zval","pval","w.random", "Ponderacion")
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "RR", "EE.RR", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
    })
      ...
      if (m$common == TRUE){
        res8dt <-
          data.frame(
            ...
            `LI[95%]` = exp(m[["lower.fixed.w"]]),
            `LS[95%]` = exp(m[["upper.fixed.w"]]),
    ...
    res_3_data <- reactive({
      ...
      if(m$common == TRUE){
        ...
        colnames(res3)=c("RR", "LI[95%]", "LS[95%]", "Z", "valor-p")
        return(res3)
      }
      if(m$common == FALSE){
    ...
    output$tables <-  renderUI({
      ...
      model_summary <- DT::formatRound(
        model_summary,
        columns = c("E.E", "N.E", "E.C", "N.C", "RR", "EE.RR",
                    "LI[95%]", "LS[95%]", "Z", "Ponderación" , "Ponderación (%)"),
        digits = 2
      )
      ...
      if (!is.null(m$subgroup)){
        ...
        tagList(
          h3(if(m$common == TRUE)
          {"Tabla 1. Riesgo Relativo con Efectos Fijos"}
          else {"Tabla 1. Riesgo Relativo con Efectos Aleatorios"}
          ),
    ...
    output$download_res1 <- downloadHandler(
      filename = function() {
        paste("tabla_riesgo_relativo _", Sys.Date(), ".csv", sep = "")
      },
...
# mod_risk_rel_server("risk_rel_1")
