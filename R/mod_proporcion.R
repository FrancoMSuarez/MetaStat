  ...
  tagList(
    bslib::layout_sidebar(
      sidebar = bslib::sidebar(
        ...
        selectInput(ns("eventos"),
                    "Seleccionar número de eventos",
                    choices = c("",NULL),
                    selected = ""),

        selectInput(ns("N"),
                    "Seleccionar número de observaciones",
                    choices = c("",NULL),
                    selected = ""),
...
#' proporcion Server Functions
#'
#' @noRd
mod_proporcion_server <- function(id, file_data){
  moduleServer(id, function(input, output, session){
    ...
    observe({
      ...
      updateSelectInput(session, "eventos", choices = c("",var_num))
    ...
    model <- eventReactive(input$run_model, {
      df <- file_data()
      req(df)
      req(input$eventos, input$N)

      metaanalisis_df <- data.frame(
        eventos = df[[input$eventos]],
        N = df[[input$N]],
      ...
      metaanalisis_df <- metaanalisis_df[
        stats::complete.cases(metaanalisis_df[, c("eventos", "N")]), ]
    ...
    res1_data <- reactive({
      ...
      if (m$common == TRUE){
        ...
        res1 <- dplyr::select( res1, "event", "n","studlab","TE","seTE","lower","upper","w.common", "Ponderacion")
        colnames(res1)=c("Evento", "n", "Estudio", "Proporción","E.E.", "LI[95%]", "LS[95%]", "Ponderación" , "Ponderación (%)")
        return(res1) }

      if (m$common == FALSE){
        res1 <- data.frame(summary(m))
        res1$Ponderacion= res1$w.random/sum(res1$w.random)*100
        res1 <- dplyr::select( res1, "event", "n","studlab","TE","seTE","lower","upper","w.random", "Ponderacion")
        colnames(res1)=c("Evento", "n", "Estudio", "Proporción", "E.E.", "LI[95%]", "LS[95%]", "Ponderación" , "Ponderación (%)")
        return(res1) }
    }
    )
    ...
    res_3_data <- reactive({
      ...
      req(m)
      if(m$common == TRUE){
        res3 <- as.data.frame(cbind(m$TE.fixed, m$lower.fixed, m$upper.fixed ))
        colnames(res3)=c("Proporción", "LI[95%]", "LS[95%]")
        return(res3)
      }
      if(m$common == FALSE){
        res3 <- as.data.frame(cbind(m$TE.random, m$lower.random, m$upper.random ))
        colnames(res3)=c("Proporción", "LI[95%]", "LS[95%]")
        return(res3)
      }
    })
    ...
    output$tables <-  renderUI({
      ...
      model_summary <- DT::formatRound(
        model_summary,
        columns = c("Evento", "n", "Proporción", "E.E.",
                    "LI[95%]", "LS[95%]", "Ponderación" , "Ponderación (%)"),
        digits = 2
      )
      ...
      res3 <- DT::formatRound(
        res3,
        columns = c("Proporción", "LI[95%]", "LS[95%]"),
        digits = 2
      )
      ...
      if (!is.null(m$subgroup)){
        ...
        tagList(
          h3(if(m$common == TRUE)
          {"Tabla 1. Estimación de una proporción con Efectos Fijos"}
          else {"Tabla 1. Estimación de una proporción con Efectos Aleatorios"}
          ),
    ...
    output$download_res1 <- downloadHandler(
      filename = function() {
        paste("tabla_proporción_", Sys.Date(), ".csv", sep = "")
      },
...
# mod_proporcion_server("proporcion_1")
