...
#' chance Server Functions
#'
#' @noRd
mod_chance_server <- function(id,file_data){
  moduleServer(id, function(input, output, session){
    ...
    res1_data <- reactive({
      ...
      if (m$common == TRUE){
        ...
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "OR", "EE.OR", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
    ...
    res_3_data <- reactive({
      ...
      if(m$common == TRUE){
        ...
        colnames(res3)=c("OR", "LI[95%]", "LS[95%]", "Z", "valor-p")
        return(res3)
      }
      if(m$common == FALSE){
    ...
    output$tables <-  renderUI({
      ...
      model_summary <- DT::formatRound(
        model_summary,
        columns = c("E.E", "N.E", "E.C", "N.C", "OR", "EE.OR",
      ...
      if (!is.null(m$subgroup)){
        ...
        tagList(
          h3(if(m$common == TRUE)
          {"Tabla 1. Cociente de Chance con Efectos Fijos"}
          else {"Tabla 1. Cociente de Chance con Efectos Aleatorios"}
          ),
    ...
    output$download_res1 <- downloadHandler(
      filename = function() {
        paste("tabla_cociente_chance_", Sys.Date(), ".csv", sep = "")
      },
