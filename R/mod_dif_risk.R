...
#' dif_risk Server Functions
#'
#' @noRd
mod_dif_risk_server <- function(id, file_data){
  moduleServer(id, function(input, output, session){
    ...
    res1_data <- reactive({
      ...
      if (m$common == TRUE){
        ...
        colnames(res1)=c("E.E", "N.E", "E.C", "N.C", "Estudio", "RD", "EE.RD", "LI[95%]", "LS[95%]", "Z", "valor-p", "Ponderación" , "Ponderación (%)")
        return(res1) }
    ...
    res_3_data <- reactive({
      ...
      if(m$common == TRUE){
        ...
        colnames(res3)=c("RD", "LI[95%]", "LS[95%]", "Z", "valor-p")
        return(res3)
      }
      if(m$common == FALSE){
    ...
    output$tables <-  renderUI({
      ...
      model_summary <- DT::formatRound(
        model_summary,
        columns = c("E.E", "N.E", "E.C", "N.C", "RD", "EE.RD",
      ...
      if (!is.null(m$subgroup)){
        ...
        tagList(
          h3(if(m$common == TRUE)
          {"Tabla 1. Diferencia de Riesgo con Efectos Fijos"}
          else {"Tabla 1. Diferencia de Riesgo con Efectos Aleatorios"}
          ),
    ...
    output$download_res1 <- downloadHandler(
      filename = function() {
        paste("tabla_Diferencia_Riesgo", Sys.Date(), ".csv", sep = "")
      },
...
# mod_dif_risk_server("dif_risk_1")
